...
public class PieChartFactory {
  ...
  public static PieChart generateDonutChart(int width, int height, int okCount, int totalCount, Font font, Color okCountColor, Color restCountColor) {
    ChartTheme matlabChartTheme = ChartTheme.Matlab; // 라벨 이름이 차트 내에 들어감
    long percent = Math.round((double) okCount / totalCount * 100.0); // Dto에서 받은거 써도 되긴함
    Color[] colors = new Color[]{okCountColor, restCountColor};
